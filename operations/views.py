...
def process_list(request):
    ...
    # Filtre mois via URL params (persistant)
    ...
    if mois and annee:
        qs = qs.filter(date_operation__month=int(mois), date_operation__year=int(annee))
    elif mois:
        qs = qs.filter(date_operation__month=int(mois))
